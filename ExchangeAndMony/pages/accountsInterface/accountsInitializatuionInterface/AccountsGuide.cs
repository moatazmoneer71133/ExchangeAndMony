...
namespace ExchangeAndMony.pages.accountsInterface.accountsInitializatuionInterface
{
    public partial class AccountsGuide : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            // عبارة عن دالة تعيد عدد الحسابات الموجودة تحت الحساب الرئيسي
            int d = tv_Accounts.SelectedNode.GetNodeCount(false);
            if (int.Parse(txt_Acc_num.Text) <= 4)
                MessageBox.Show("لايمكن حذف هذا الحساب لانه من الحسابات الرئيسية");
            else if ((d == 0))
            {
                // If SearchFor_Data_In_Tbl(AccID.Text, "Operations", "OpAccID", " DB/ *E ',1'! -1C'* 9DI G0' 'D-3'( D0DC D'JECF -0AG  ") = True Then Exit Sub
        ...
        {
            //MessageBox.Show(com_Acc_Type.SelectedIndex.ToString());
            try
            {
                 if (com_Acc_Type.SelectedIndex == 0 && com_Acc_Parent.Text.Trim() == "")
                {
                  //  MessageBox.Show("com_Acc_Type.SelectedIndex == 0 && com_Acc_Parent.Text.Trim() 1== \"\"");
                    this.txt_Acc_num.Text = xx.GetAccCode(0, this).ToString();
                }
                else if (com_Acc_Type.SelectedIndex == 0 && com_Acc_Parent.Text.Trim() != "")
                {
                   // MessageBox.Show("com_Acc_Type.SelectedIndex == 0 && com_Acc_Parent.Text.Trim() 2 != \"\"");
                    //this.txt_Acc_num.Text = Convert.ToString(xx.GetAccCode(Convert.ToInt64(this.txt_parent_num.Text), this));
                    this.txt_Acc_num.Text = Convert.ToString(xx.GetAccCode(Convert.ToInt64(this.com_Acc_Parent.SelectedValue), this));
                   // MessageBox.Show("com_Acc_Type.SelectedIndex == 0 && com_Acc_Parent.Text.Trim() 22 != \"\"");
                }
                else if (com_Acc_Type.SelectedIndex == 1 && com_Acc_Parent.Text.Trim() != "")
                {
                    ...
                    //this.txt_Acc_num.Text = Convert.ToString(xx.GetAccCode(Convert.ToInt64(this.txt_parent_num.Text), this));
                    this.txt_Acc_num.Text = Convert.ToString(xx.GetAccCode(Convert.ToInt64(this.com_Acc_Parent.SelectedValue), this));
                   // MessageBox.Show("com_Acc_Type.SelectedIndex == 1 && com_Acc_Parent.Text.Trim() 33 != \"\"");
                }
            }
